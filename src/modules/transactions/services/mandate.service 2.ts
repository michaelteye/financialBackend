...
export class MandateService extends HttpRequestService {
  ...
  async create(createMandateDto: CreateMandateDto): Promise<any> {
    ...
    mandate.endDate = createMandateDto.startDate as Date;
    ...
    const retResponse = { status: "", message: "", mandateId: postdata.reference };
  ...
  async deactivate(reference: string): Promise<any> {
    const mandate = await this.mandateRepository.findOne({ where: { reference } });
    if(mandate.status==MandateStatus.DEACTIVATED || mandate.status==MandateStatus.FAILED){
      return;
    }
