...
export class SavingGoalCronService {
    ...
    async resolveInactiveUsersWithinTwoMonths() {
        this.logger.log('------- Inactive Customers within two months running ---------');
        ...
        try {
            ...
            if (data.length != 0) {
                for (let k = 0; k < data.length; k++) {
                    ...
                    if (sequenceManager.status === CRON_STATUS.INPROGRESS && sequenceManager.count < 5) {
                        // Send the customer an SMS before sending the auto debit prompt.
                        let firstName = data[k].firstName ? data[k].firstName : "Bezo Customer";
                        let mobile = data[k].phone;

                        await this.notificationService.sendSms({
                            to: [mobile],
                            sms: `Dear ${firstName}, Kindly make an initial deposit of GHS5 to activate your account. Thank you!`,
                        });
    ...
    */
        ...
        for (let k = 0; k < savingsGoals.length; k++) {
            ...
            if (account.balance >= savingsGoal.amountToRaise) {
                ...
                this.notificationService.sendSms({
                    to: ['233594951335'],
                    sms: 'Hello Customer, you have successfully completed'
     ...
     */
    @Cron(CronExpression.EVERY_10_SECONDS) // Run this every week.
    async unlockBezoLockSavingsGoalsForWithDrawal() {
        ...
        let savingsGoals = await this.getInProgressBezoLockSavingsGoalsOlderThanAMonth();
        for (let k = 0; k < savingsGoals.length; k++) {
            let lockedSavingsGoal = savingsGoals[k];
            //Release locked account to allow Withdrawals with fees
            await this.em.query(`update account_entity set "allowWithdrawalWithFees"=true where id='${lockedSavingsGoal.accountId}'`)
        }
    }
    ...
    //     console.debug('------- Customers Celebrating Birthdays Today Running ---------');
    //     const customers = await this.getCustomersWhoseBirthdaysFallToday();
    //     let phoneNumbers = [];
    //     for(let k = 0; k < customers.length; k++) {
    //         // console.log(`${customers[k].firstName} ${customers[k].phone}`);
    //         phoneNumbers.push(customers[k].phone);
    //     }
    ...
    async getCustomersWhoseBirthdaysFallToday() {
        return await this.em.query(`SELECT * FROM public.user_entity AS ue JOIN public.auth_user_entity 
    ...
    async calculateSavingsGoalsLikelyToFail(savingsGoal: SavingsGoalEntity, account: AccountEntity) {
        let endDate = parseISO(format(new Date(savingsGoal.endDate), 'yyyy-MM-dd'));

        let startDate = parseISO(format(new Date(savingsGoal.startDate), 'yyyy-MM-dd'));

        this.logger.log(`Start Date -----> ${startDate}`);

        this.logger.log(`End Date ------> ${endDate}`);

        let duration = intervalToDuration({ start: new Date(endDate), end: new Date(startDate) });

        let days = subMonths(endDate, 3);

        let threeMonthsBehind = intervalToDuration({ start: new Date(days), end: new Date() });

        this.logger.log(`Three months behind --->  ${threeMonthsBehind}`);

        console.log(`Savings goal days ---> ${duration}`);

        if (duration.months < threeMonthsBehind.months && account.balance < savingsGoal.amountToRaise) {
    ...
    async getCustomerInfoById(userId: string) {
        return this.em.query(`
           ...
           ON aue."userId" = ue."id" AND ue."id"=?`, [userId]);
    }

    async getSavingsGoalsInProgressEndingToday() {
        return await this.em.find(SavingsGoalEntity, {
            where: {
                endDate: new Date(), //less than or equal to today
                goalStatus: "INPROGRESS"
            }
        });
    }

    async getInProgressBezoLockSavingsGoalsOlderThanAMonth(): Promise<SavingsGoalEntity[]> {
    ...
    async getCustomersMobileNumbers(userId: string) {
        try {
            const result = await this.em.query('select * from public.auth_user_entity where "userId"=?', [userId]);
            console.log(result);
            return result;
        } catch (e) {
            console.log(e);
    ...
    async updateCompletedGoalStatus(userId: number, goalStatus: string) {
        return this.em.update(SavingsGoalEntity, userId, { goalStatus: goalStatus })
    }
    ...
    async getCustomersWithZeroBalance(): Promise<any> {
        return await this.em.query(
            `
            SELECT ua."id" AS userId, pm."network" AS network, ae."id" AS accountId, aue."phone" AS phone, ua."firstName" AS firstName
            ...
            WHERE ae."balance" = 0 AND ua."createdAt" >= NOW() - INTERVAL '1 WEEK'
            `
        );
    }

    async deposit(userId: string, accountId: string, phone: string, amount: number, network: string, description: string): Promise<any> {
        ...
        let depositResponse = await this.transactionService.callPaymentGateWayForDepositWithdrawal(
            phone,
            amount,
            description,
        ...
        this.logger.log(depositResponse);
