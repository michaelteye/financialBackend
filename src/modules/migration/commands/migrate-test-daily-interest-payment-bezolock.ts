...
@Console()
export class MigrateTestDailyInterestPaymentBezoLockCommand {
  ...
  @Command({
    command: 'migrate:daily-interest-payment-bezo-lock',
  })
  ...
  async _execute(opts?: any) {
   ...
   // const transactions =  await this.InterestPaymentService.payBezoFlexUsers()
    ...
    const transAccountSavingBeforeFeb2023= await this.InterestPaymentService.dailyInterestCalcForBezoLockUsers()
